...
public class ShoppingLogService {
    ...
    public Map<String, Object> processPayment(String uid, String spno, String cardno, String slamount, String slcurrency) {
        ...
        if (!"Y".equals(card.getCardstatus())) {
            ShoppingLog failedLog = saveFailedLog(userOpt.get(), productOpt.get(), card, slamount, slcurrency, "2000");
        ...
        BigDecimal currentBalance = new BigDecimal(childAccount.getCabalance());
        ...
        childAccount.setCabalance(newBalance.toString());
        childAccountRepository.save(childAccount);
        
        History history = History.builder()
            .hno(generateHistoryNumber())
            .uid(uid)
            .paaccount(childAccount.getParentAccount().getPano())
            .caserial(childAccount.getCurrency().getCuno())
            .hwithdraw(slamount)
            .hdeposit("0")
    ...
    private Long generateHistoryNumber() {
