...
public class CardService {
    ...
    public CardDto getCardByUserId(String uid) {
        Card card = cardRepository.findByUserUid(uid)
                .orElseThrow(() -> new RuntimeException("카드 정보를 찾을 수 없습니다."));
        
        CardDto cardDto = cardConverter.toDto(card);
        cardDto.setCardno(maskCardNumber(card.getCardno()));
    ...
    @Transactional
    public boolean reportLostCard(String uid) {
        Card card = cardRepository.findByUserUid(uid)
                .orElseThrow(() -> new RuntimeException("카드 정보를 찾을 수 없습니다."));
        
        card.setCardstatus("L");
        cardRepository.save(card);
        return true;
    }
    
    @Transactional
    public boolean cancelLostReport(String uid) {
        Card card = cardRepository.findByUserUid(uid)
                .orElseThrow(() -> new RuntimeException("카드 정보를 찾을 수 없습니다."));
        
        card.setCardstatus("Y");
        cardRepository.save(card);
        return true;
    }
}